...
多线程语言的 http server 为了提高性能，会使用多线程来处理请求，线程往往就像 worker 一样处理着请求，**一般**来说一个请求整个生命周期会在同一个线程中，那么此时，使用 `ThreadLocal` 来传递请求上下文，tracing 信息之类的就很方便了。也就是我们可以不用向 `golang` 那样显式传递这些上下文信息。
